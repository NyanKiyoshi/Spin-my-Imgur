...
function getScaleFactor() {
    var scale_factor = parseFloat(settings.prefs['scale_factor']);
    if (isNaN(scale_factor)) {
        // if `scale_factor` is invalid, replace the value by "4.1"
        settings.prefs['scale_factor'] = "4.1";
        scale_factor = 4.1;
    }
    return scale_factor;
}
...
pageMod.PageMod({
    ...
    onAttach: function(worker) {
        worker.port.emit("scale_factor", getScaleFactor());
    }
});
...
// exports unload function which this one will unload the observer
